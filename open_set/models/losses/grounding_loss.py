...
class GroundingLoss(nn.Module):
    ...
    def forward(self,
                ...
                gt_caption_noun_embs: torch.Tensor,
                gt_caption_noun_mask: torch.Tensor,
                temperature: float,
                **kwargs):
        """Forward function.
        ...
        loss_grounding = self.loss_weight * self._grounding_loss(
            cls_emb_pred,
            gt_caption_noun_embs,
            gt_caption_noun_mask,
            temperature,
            **kwargs)
        return loss_grounding
    
    def _grounding_loss(self, cls_emb_pred: torch.Tensor, gt_caption_noun_embs: torch.Tensor, gt_caption_noun_mask: float, temperature: float) -> float:
        ...
        Args:
            ...
            gt_caption_noun_embs: A tensor of shape (batch_size, max_tokens, d_l) that stores the word embeddings of the caption.
            gt_caption_noun_mask: A tensor of shape (batch_size, max_tokens) that stores the mask of the word tokens.
        ...
        if (gt_caption_noun_mask.sum(dim=1) > 0).sum() < batch_size:
            ...
            cls_emb_pred = cls_emb_pred[None, :, :, :].repeat(batch_size, 1, 1, 1).reshape(batch_size**2, num_queries, d_l)   #[bv1, bv2,...bvB, bv1, bv2, ...]
            gt_caption_noun_embs = gt_caption_noun_embs[:, None, :, :].repeat(1, batch_size, 1, 1).reshape(batch_size**2, num_max_tokens, d_l)  #[bl1, bl1,...bl1, bl2, bl2, ...]
            gt_caption_noun_mask = gt_caption_noun_mask[:, None, :].repeat(1, batch_size, 1).reshape(batch_size**2, num_max_tokens)  #[bl1, bl1,...bl1, bl2, bl2, ...]
            num_tokens = num_tokens[:, None].repeat(1, batch_size).reshape(batch_size**2)  # [#nouns1, #nnouns1, ....#nouns2, #nnouns2, ...]
...
class GroundingLossWithSparistyConstrain(GroundingLoss):
    ...
    def _grounding_loss(self, cls_emb_pred: torch.Tensor, gt_caption_noun_embs: torch.Tensor, gt_caption_noun_mask: float, temperature: float) -> float:
        ...
        Args:
            ...
            gt_caption_noun_mask: A tensor of shape (batch_size, max_tokens) that stores the mask of the word token.
        ...
        gt_caption_noun_mask = torch.cat([gt_caption_noun_mask, torch.ones((batch_size, 1),device=gt_caption_noun_mask.device)], dim=1)  # None description should be attended to
        _, num_max_tokens = gt_caption_noun_mask.shape
        num_tokens = gt_caption_noun_mask.sum(dim=1)  # batchsize
        ...
        gt_caption_noun_mask = gt_caption_noun_mask[:, None, :].repeat(1, batch_size, 1).reshape(batch_size**2, num_max_tokens)
        ...
        attention_l2v = F.softmax(local_similarity, dim=2)
        attention_l2v = attention_l2v * gt_caption_noun_mask[:, :, None]
        ...
        return (1 - self._sparsity_loss_relative_weight) * similarity_loss + \
            self._sparsity_loss_relative_weight * self._v2l_attention_sparsity_loss(
                ...
                noun_mask=gt_caption_noun_mask,
