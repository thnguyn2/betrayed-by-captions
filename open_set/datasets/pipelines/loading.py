...
class LoadOpenAnnotations:
    """Load multiple types of annotations in open setting.
    ...
    def _load_caption(self, results):
        """Private function to load the category name of label annotations.
        Used for turning labels into embeddings.
        ...
        results['gt_caption_ids'] = results['ann_info']['caption_ids'].copy()
    ...
    def __call__(self, results):
        ...
        if self.with_caption:
            results = self._load_caption(results)
        return results
...
@PIPELINES.register_module()
class LoadOpenPanopticAnnotations(LoadAnnotations):
    ...
    def __init__(self,
        ...
        super(LoadOpenPanopticAnnotations, self).__init__(
            with_bbox=with_bbox,
        ...
        # whether to load box based on thing mask
        self.with_mask_based_bbox = with_mask_based_bbox
        self.with_caption = with_caption
    ...
    def _load_masks_and_semantic_segs(self, results):
        ...
        # print("gt mask len:", len(results['gt_masks']))
    ...
    def __call__(self, results):
        ...
        if self.with_bbox and not self.with_mask_based_bbox:
            results = self._load_bboxes(results)
            if results is None:
                return None
        if self.with_label:
