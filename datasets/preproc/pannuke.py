...
def _generate_dataset() -> None:
    ...
    _ORG_DATASET_FOLDER = _ROOT_FOLDER / "Fold 1"
    images = np.load(_ORG_DATASET_FOLDER/"images"/"fold1"/"images.npy").astype(np.uint8)
    image_types = np.load(_ORG_DATASET_FOLDER/"images"/"fold1"/"types.npy")
    masks = np.load(_ORG_DATASET_FOLDER/"masks"/"fold1"/"masks.npy")
    dir_by_name = _create_output_dataset_folders(target_folder_path=_ROOT_FOLDER / "pannuke_coco")
    
    starting_annotation_id = 1
    all_annotation_dicts = []
    image_infos = []
    image_id = 1
    for image, _, mask in tqdm(zip(images, image_types, masks)):
        ...
        for class_id in _CLASS_NAMES_BY_IDS:
            if class_id != _BG_CLASS_ID:
                ...
                all_annotation_dicts.extend(single_channel_annotation_dicts)
                    ...
                    annotation_dicts.append(
                        {
                            ...
                            'id': starting_annotation_id + offset_idx,
